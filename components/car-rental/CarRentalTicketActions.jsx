...
export default function CarRentalTicketActions({ ticketCode }) {
  const [toast, setToast] = useState(null);

  const handleDownload = () => {
    setToast(`Đang chuẩn bị file PDF cho xác nhận ${ticketCode}...`);
    window.setTimeout(() => setToast(null), 2200);
  };

  const handleResendEmail = () => {
    setToast("Đã gửi lại xác nhận thuê xe đến email của bạn.");
    window.setTimeout(() => setToast(null), 2200);
  };

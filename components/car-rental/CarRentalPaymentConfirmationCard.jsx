...
export function getCarRentalConfirmationStorageKey(vehicleId) {
  return `viego:car-rental:confirmation:${vehicleId}`;
}

export default function CarRentalPaymentConfirmationCard({
  vehicleId,
  bookingCode,
  nextQuery,
}) {
  ...
  const handleConfirm = () => {
    ...
    router.push(
      `/car-rental/booking/${encodeURIComponent(vehicleId)}/confirmation${
        nextQuery ? `?${nextQuery}` : ""
      }`
    );
  };
    ...
    <section>
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-5">
        <p className="text-sm text-slate-600 mb-4 leading-relaxed">
          Sau khi xác nhận thanh toán của bạn, VieGo sẽ gửi xác nhận thuê xe
          qua email và cập nhật trong mục Đặt chỗ của tôi.
        </p>
        <button
          type="button"

...
export default function CarRentalPaymentCountdownBanner({
  vehicleId,
  ...
  useEffect(() => {
    const storageKey = `viego:car-rental:payment:expiresAt:${vehicleId || "default"}`;
    let expiresAt;
    try {
  ...
  return (
    <div className="bg-sky-600 text-white rounded-2xl px-5 py-4 flex items-center justify-center gap-3">
      <p className="text-sm md:text-base font-medium text-center">
        {expired ? (
          <>Thời gian giữ giá đã hết. Vui lòng quay lại tìm kiếm để đặt lại xe.</>
        ) : (
          <>
            Đừng lo lắng, giá vẫn giữ nguyên. Hoàn tất thanh toán của bạn bằng{" "}
            <span className="font-bold text-emerald-300">
              {"00:" + formatClock(secondsLeft)}
            </span>
          </>
        )}
      </p>
      <svg
        viewBox="0 0 24 24"
        className="w-5 h-5 shrink-0 text-emerald-300"
        fill="none"
        stroke="currentColor"
        strokeWidth="2"
        strokeLinecap="round"

...
import CarRentalContactForm from "./CarRentalContactForm";
import CarRentalPassengerForm from "./CarRentalPassengerForm";
...
function validate(values) {
  ...
  if (!values.passengerPhone.trim())
    errors.passengerPhone = "Vui lòng nhập số điện thoại.";
  else if (!/^\d{8,}$/.test(values.passengerPhone.trim()))
    errors.passengerPhone = "Số điện thoại không hợp lệ.";
...
export default function CarRentalBookingClient({ vehicleId, searchQuery }) {
  ...
  useEffect(() => {
    try {
      const raw = sessionStorage.getItem(getStorageKey(vehicleId));
      if (raw) {
  ...
  const handleSubmit = (e) => {
    ...
    persist(values);
    router.push(
      `/car-rental/booking/${encodeURIComponent(vehicleId)}/review${

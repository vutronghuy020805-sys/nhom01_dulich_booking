...
import { applyCarRentalCoupon } from "@/data/carRentalRental";

export default function CarRentalCouponCard({
  subtotal,
  appliedCoupon,
  onApply,
  onRemove,
  ...
  const handleApply = () => {
    const result = applyCarRentalCoupon(code, subtotal);
    if (!result.valid) {
    ...
    <section className="bg-white rounded-2xl border border-slate-200 p-5">
      ...
      {open && !error && (
        <div className="mt-2 text-xs text-slate-500">
          Thử mã: <code className="font-semibold">VIEGOCAR10</code>,{" "}
          <code className="font-semibold">VIEGOCAR50</code>,{" "}
          <code className="font-semibold">VIEGOBANMOI</code>
        </div>
      )}
    </section>
  );

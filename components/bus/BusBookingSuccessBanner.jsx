...
import { getBusConfirmationStorageKey } from "./BusPaymentConfirmationCard";
...
export default function BusBookingSuccessBanner({
  tripId,
  bookingCode,
  ticketCode,
}) {
  ...
  useEffect(() => {
    try {
      const raw = sessionStorage.getItem(getBusConfirmationStorageKey(tripId));
    ...
    <section className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
      <div className="px-6 py-8 text-center bg-gradient-to-b from-emerald-50 to-white">
        ...
        <p className="mt-2 text-slate-600 max-w-xl mx-auto leading-relaxed">
          Vé điện tử của bạn đã được tạo thành công. VieGo đã gửi thông tin đặt
          chỗ đến email của bạn. Bạn cũng có thể xem lại vé này trong mục{" "}
